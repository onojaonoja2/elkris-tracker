        ...
        @else
            <table class="w-full text-sm text-left">
                ...
                <tbody>
                        ...
                        <tr class="border-b">
                            ...
                            <td class="px-3 py-2">
                                @if ($record->assignedTo)
                                    <div class="font-medium">{{ $record->assignedTo->name }}</div>
                                    @if ($record->assignedTo->state?->name || $record->assignedTo->lga?->name)
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ collect([$record->assignedTo->lga?->name, $record->assignedTo->state?->name])->filter()->implode(', ') }}
                                        </div>
                                    @endif
                                @else
                                    -
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 py-4 text-center text-gray-500">No records found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
