...
class AccountantCustomersWidget extends TableWidget
{
    ...
    protected function getDefaultDateRange(): array
    {
        ...
        $preset = session()->get('accountant_customer_date_preset', 'lifetime');

        $from = match ($preset) {
            'today' => $now->copy()->setHour(8)->setMinute(0)->setSecond(0),
            'yesterday' => $now->copy()->subDay()->startOfDay(),
            'this_week' => $now->copy()->startOfWeek(),
            'this_month' => $now->copy()->startOfMonth(),
            default => Carbon::now('Africa/Lagos')->subYears(10),
        };
